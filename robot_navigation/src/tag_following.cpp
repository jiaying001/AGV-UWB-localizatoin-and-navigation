...
void scanCallback (const sensor_msgs::LaserScan::ConstPtr& scan_msg)
{
  ...
  if (att_Amp > 0.20) 		//not using repul_Angle here because obstacles may exist near the target point
  {
...
**/
  ...
  {
    angle_diff = atan2(sin(angle - angle_tgt), cos(angle - angle_tgt));
    if (angle_diff > 0.2) 
    {
      vel_pub_msg.linear.x = 0.00;
      vel_pub_msg.angular.z = -0.15;
      vel_pub.publish(vel_pub_msg);
    } 
...
{  
  double dx = 0; 
  double dy = 0; 
  dx = fabs(x_tgt - x_tgt_last)/100.0; 
  dy = fabs(y_tgt - y_tgt_last)/100.0; 
  if ((dx > 0.20)||(dy >0.20))
    return 1; 
  else
    return 0;
...
{
  ros::init(argc, argv, "tag_following");
  ros::NodeHandle n;
